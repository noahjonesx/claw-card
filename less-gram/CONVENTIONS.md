...
- `feature/<short-name>` — new features (e.g. `feature/dm-tab`)
- `fix/<short-name>` — bug fixes (e.g. `fix/reels-flash`)
- `chore/<short-name>` — non-functional changes (e.g. `chore/update-conventions`)

No direct pushes to `main`. Open a PR, get one approval.
